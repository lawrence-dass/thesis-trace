...
5. ~~**Score-teaching UI**~~ — **done 2026-07-22 (company page), extended 2026-07-25 (Compare page):** per-model range/zone gauges + plain-language captions, so a visitor sees not just the number but what it means and which direction is favorable, on both the company page's Verdict cards and the Compare table. Same `Gauge` component (`frontend/app/components/ui/Gauge.tsx`), same live-from-backend band data on both pages.
