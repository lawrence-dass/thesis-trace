...
2026-07-29 (`main` @ `caf4a31`). Every label traces to a real file, route, table, or
...
2. [03-data-pipeline.md](./03-data-pipeline.md) — the deterministic write path: EDGAR / Tiingo / Bank of Canada → `raw_facts` → AD-3 canonicalization → identity validation → four scoring models → append-only `score_runs`.
...
| Batch pipeline | `pipeline/run.py` — ingest → canonicalize → validate → score | Render Cron Job `thesistrace-pipeline`, `0 6 * * *`; the daily run doubles as the Supabase free-tier keep-alive |
...
| Tests | pytest 8.3 (`asyncio_mode = auto`) | 57 test functions across 14 files, incl. `test_golden_dataset.py` against 4 hand-verified companies |
...
## Note on the validate stage

The first version of this suite flagged a real gap: `run_validation` was implemented and
tested but never called by `pipeline/run.py`, so the validate stage existed only in the test
suite. That is now fixed — it runs between canonicalize and score, advisory-only, and is
idempotent so the daily cron cannot accumulate duplicate `data_quality_issues` rows. See
[03-data-pipeline.md](./03-data-pipeline.md#the-validate-stage).
