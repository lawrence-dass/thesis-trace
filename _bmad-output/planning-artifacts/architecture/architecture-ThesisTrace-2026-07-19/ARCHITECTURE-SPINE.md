...
- **Extended 2026-09-10 (Story 13.1) for DIMENSIONED facts.** The Company Facts API carries no dimensional data whatsoever — not sparsely, not by exception; segment- and member-level facts are absent by design, verified live against a 4.6 MB fetch (`company_facts_api_carries_no_segment_dimensions`). So for a dimensioned fact Inline XBRL is not a *fallback* but the sole source, and the conflict clause above is **inert by construction**: there is never an overlapping Company Facts candidate to conflict with, and a dimensioned fact is never the "same fact" as its undimensioned counterpart (AD-3 rule 0). Consequently a dimensioned fact carries no `data_quality_issues` divergence row on ingestion, and the "Company Facts wins" precedence has no application to it. Dimensions form part of a fact's identity in `raw_facts` (they are hashed into `content_hash`, or two members reporting an equal value collide on `(accession_number, content_hash)` and one is silently dropped).
