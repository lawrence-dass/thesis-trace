...
- **Rule:** selection order is **(0) a fact carrying DIMENSIONS is not a candidate for an undimensioned canonical concept at all** — it is a different fact, excluded before ranking begins — then (1) as-originally-filed over restated comparative, (1a) within the originally-filed tier, an amendment of that same fiscal year (10-K/A, 40-F/A) over the original it amends — the filer's own supersession, which AD-6 depends on, not a guess; two competing amendments of one year remain ambiguous — (2) higher decimals precision. Any unresolved ambiguity writes a `data_quality_issues` row with status `needs_review` — never a defaulted guess. *(1a added 2026-09-02 closing `canonical_facts_amendment_gap`: without it AD-6's "referencing the new canonical_facts" was unsatisfiable, since an amendment's restated value could only ever tie with — and be flagged against — the original.)*
- **Rule 0 replaces the original rule (2), "least-dimensioned/most-specific member" (Story 13.1, 2026-09-10).** That clause assumed a single flat candidate pool in which a dimensioned fact could legitimately compete and simply rank lower. It was never implemented, and never needed to be, because the only production source — the Company Facts API — carries no dimensional data at all. It therefore read as enforced from 2026-07-19 until Story 13.1, over a year. Ranking is the wrong mechanism: a segment's revenue is not a worse measurement of consolidated revenue, it is a measurement of something else. Two failure modes were reproduced against the real canonicalizer before the guard landed — a dimensioned fact that DISAGREES opens a spurious `ambiguous_selection` (which also suppresses the consolidated fact entirely), and a dimensioned fact that is the SOLE candidate for a filer-year silently becomes the canonical consolidated value with no flag raised, the same lone-candidate blind spot as `otex_capex_sign_error_fy2007_fy2009`. Enforced by `backend/tests/test_dimensional_fact_contract.py`. A dimensioned fact may enter candidacy only through an explicit axis/member-scoped mapping for a dimension-aware canonical concept; no generic consolidated or total-segment member is a fallback. The current `canonical_facts` key cannot represent multiple member facts for one issuer/concept/year/mapping_version, so member-aware facts require a separate store or key extension (Story 13.4).
...
- **Rule:** SEC Company Facts API is the primary ingestion source; raw Inline XBRL parsing is the audit/fallback path for facts the Company Facts API omits **and the only path to facts it structurally cannot carry**. On a value conflict between the two sources for the same fact, Company Facts wins and the divergence writes a `data_quality_issues` row (AD-17); Inline XBRL only supplies facts Company Facts omits, never silently overrides.
- **Extended 2026-09-10 (Story 13.1) for DIMENSIONED facts.** The Company Facts API carries no dimensional data whatsoever — not sparsely, not by exception; segment- and member-level facts are absent by design, verified live against a 4.6 MB fetch (`company_facts_api_carries_no_segment_dimensions`). So for a dimensioned fact Inline XBRL is not a *fallback* but the sole source. For a dimensional-vs-undimensioned pair, the conflict clause above does not apply: there is no Company Facts dimensional counterpart, and a dimensioned fact is never the "same fact" as its undimensioned counterpart (AD-3 rule 0). This is a contract for Story 13.2's future Inline ingestion path, not a claim that source-conflict detection is implemented by this story; if that path produces genuinely overlapping same-identity facts, it must apply the existing conflict policy. Dimensions form part of a fact's identity in `raw_facts` (they are hashed into `content_hash`, or two members reporting an equal value collide on `(accession_number, content_hash)` and one is silently dropped).
