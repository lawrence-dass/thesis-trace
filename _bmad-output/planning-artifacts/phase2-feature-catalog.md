...
| Q1.6 | **Capex vs. D&A** (maintenance starvation). Several years of capex < D&A inflates FCF now and is paid for later; a reverse DCF on starved FCF flatters the stock. | capex (mapped, Epic 6), `DepreciationDepletionAndAmortization`; IFRS `DepreciationAndAmortisationExpense` | **8/9** — SU has no capex tag filed at all, permanently `insufficient_data` for this signal; OTEX only FY2010-2019 (D&A tag gap after FY2019 + a live capex sign defect FY2007-2009, see `otex_capex_sign_error_fy2007_fy2009`) | **CP** (clean, FY2008-2025); OTEX FY2010-2019 only — SU, half the original rationale, cannot compute this signal at all | Epic 6 already stores capex. Cheapest item in the catalog. | ≤1 | **yes (2026-09-02)** |
...
| Q3.3 | **Achieved-vs-implied growth** — the historical growth ThesisTrace already can compute against the growth the price implies. The one-line version of Q3.1 that needs no external table. | Revenue / FCF CAGR over the stored window vs. reverse-DCF implied rate | 9/9 | ZTS, SHOP | Uses only shipped data. Distinct from Trajectory (per-model score direction). | ≤1 | no |
