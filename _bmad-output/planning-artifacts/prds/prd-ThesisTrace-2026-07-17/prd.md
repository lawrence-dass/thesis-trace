...
1. ~~Golden-dataset sourcing~~ — **resolved 2026-07-29:** sourced by hand-verification rather than published third-party scores. Every canonical input for all four companies was independently cross-checked against that company's raw EDGAR company-facts JSON (bypassing the pipeline and DB entirely), then all four models were hand-computed from those confirmed figures and matched the pipeline's stored output exactly — Piotroski to the signal, Beneish to all 8 indices, Altman to all 5 components. Lives in `backend/tests/golden/phase1_golden.yaml` and is enforced by `test_golden_dataset.py`. **SM-1 is now measurable.** Currency as new filings arrive is handled by AD-6's append-only supersession, but the hand-verified expectations themselves are manual — re-verification is required whenever a new fiscal year enters the dataset.
...
9. ~~**Lens sub-metric depth**~~ — **resolved 2026-08-04: adopt the two that are not duplicative; drop the two that are.**
   ...
   |---|---|---|
   | Debt maturity concentration | Quality/Health | **Adopt, redefined 2026-08-04** → *near-term debt share* (see below) |
   ...
   **Refinement 2026-08-04 — "debt maturity concentration" → "near-term debt share".** A live EDGAR verification spike (Epic 5 Story 5.1) checked all seven filers before anything was built. Only QSR and CP carry a usable year-by-year maturity ladder; OTEX never tags years 2–5 and switched its near-term tag twice mid-history; SHOP tags a lease ladder but no debt one; and CCJ, BCE and SU carry **none**, for a structural reason rather than a tagging-quality one — under IFRS the maturity analysis is a liquidity-risk disclosure carried on axis/member **dimensions**, and EDGAR's company-facts API exposes only non-dimensional facts, so it is unreachable by the current ingestion path entirely.

   The metric is therefore **redefined rather than restricted**: **near-term debt share** = debt due within twelve months ÷ total debt. This is a single concept, identically defined whether read from a ladder's first bucket or from a current/noncurrent split, so it remains comparable across filers with no caveat — and it resolves for six of seven with usable history (SHOP is thin, FY2023–2024). The full year-by-year profile is demoted to a **display enrichment** for the two filers that publish one, shown as extra detail where it exists and never as something the other five lack. Full spike record in `engineering-findings.yaml` under `story_5_1_debt_maturity_spike`.
